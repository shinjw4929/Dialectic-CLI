...
- §3 출력 (생성·변경 파일 + 시그니처 수준 명세)
  - phase 파일 §3 코드 블록의 paste/spec 라벨 검사 (review-plan-checklist §1.3 "§3 코드 블록 라벨" 행) — paste 의도인 정의에 라벨 없으면 P1 보고. 자세히는 `plan-writing-guide.md` §3.1.
...
- 사용자 신뢰만으로 통과 X.
- **절대 날짜·요일 라벨 발견 시 P1 결함 보고** — plan 본문에 `2026-05-08`, `5/9 토`, `목요일` 같은 표기가 있으면 외부 calendar와 어긋날 risk. Day index + 가용 시간 + 마일스톤 추상 표현으로 정정 권고. 자세히는 `plan-writing-guide.md` §5 안티패턴.
