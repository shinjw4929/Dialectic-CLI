...
| `src/ui.py` (사용자 개입 UI — `TriggerListener`/`prompt_decision`/`prompt_end_or_iterate` 등 raw mode 자산) | `outline/03-ux.md` §3.1·§3.2·§3.3 (5 위치 cascade — default 분기·강도 dial·자동 진행 예시·수렴 종료 mermaid), `docs/dev-docs/code-conventions.md` §7 (TriggerListener 패턴), `docs/dev-docs/validation.md` P-RAW (raw mode 결함 패턴) |
