...
| `DIALECTIC_REPO_ROOT` | `Path(__file__).resolve().parent.parent` | ADR-6 차단 기준 — `is_under_repo_root` predicate가 본 상수와 비교 (모듈 top SSOT, cwd-isolation §Layer 2) |
...
### `is_under_repo_root(path: Path) -> bool` (ADR-6)

```python
return path == DIALECTIC_REPO_ROOT or DIALECTIC_REPO_ROOT in path.parents
```

`run_session` 진입 검증(`SystemExit` final gate) + cli `_input_workdir` UX-level 조기 거부(mkdir 전) 두 호출자가 본 helper로 동일 판정. 호출자 책임으로 `path = path.resolve()` 권장 — relative path는 cwd 기준 해석되며 repo 안에서 호출 시 `Path("foo").resolve()`가 repo 하위로 떨어짐. 차단 narrative 본문은 [`cwd-isolation.md` §Layer 2](cwd-isolation.md).
...
| 서브커맨드 | 인자 |
|---|---|
| `run` | `--task`(필수), `--workdir`, `--driver`, `--reviewer`, `--max-turns`, `--mode`, `--convergence-streak`, `--interactive`, `--spec` |
| `implement` | `--spec`(필수), `--workdir`, `--driver`, `--reviewer`, `--max-turns`, `--convergence-streak`, `--interactive` (alias — `set_defaults(mode="implement", task="")`, plan 014) |
| `doctor` | (인자 없음) |
| `logs` | `--workdir`, `--session`, `--tail`, `--follow`, `--kind`, `--full` (plan 010 Phase A) |
| (default) | 인자 0 → `_interactive_menu()` 진입 |

`--mode choices=["run", "plan", "implement"]` (plan 011 Phase A에서 plan/implement 노출, plan 014에서 implement wiring 활성. compare는 별도 subcommand 필요). `MODE_ROLES` dict는 이미 plan/implement 키 보존.
