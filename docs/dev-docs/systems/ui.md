...
사용자 개입 UI 자산 모음 (A 층 메뉴 진입 + B 층 turn loop 호출). plan 006-ui (Spinner·prompt_decision 신설) → plan 008-ui-polish (print_message·stdin_canonical_off·stdin_utf8_mode·flush_stdin 추가) → plan 009-user-synthesis-wiring (TriggerListener·prompt_end_or_iterate 추가) 누적 산출.
...
핵심 정합성: 외부 의존성 0 (표준 라이브러리만 — `os/sys/threading/select/termios/tty/signal/fcntl/time/contextlib/typing`). isatty 가드로 CI·파이프 환경 silent fallback.
...
| `prompt_end_or_iterate(turn_id, reason, *, allow_continue=True, allow_iterate_no_directive=True)` | 함수 | Y/n/c/text 분기 → `(key, directive_or_None)`. allow 인자별 옵션 노출 분기 (trigger 단독→Y/c/text, CONVERGED/last_turn→Y/n/text). retry 3회 한계 fallback (`c` 또는 `e`). EOF/KBI=`("e", None)` | critical 모드 종료 직전 / 트리거 발동 / max-turns 도달 | input() (readline lib 우회 X) — cleanup-restart 패턴이라 byte 절도 race 0 |
...
| `TriggerListener()` | 클래스 (컨텍스트) | Ctrl+F (`0x06`) raw mode listener. `__enter__` setcbreak (`when=TCSANOW` — drain X) + 사전 누름 회수 (fcntl `O_NONBLOCK` + `os.read` drain → 0x06 검사). `_run` thread `select` fd-level + `os.read` byte 단위 + self-pipe wake. trigger.set 후 thread 계속 (loop 유지 — 추가 누름 안내). `__exit__` self-pipe wake + thread join + tcsetattr(`TCSADRAIN`) 복원 + `tcflush(TCIFLUSH)` drain (Ctrl+F 연타 잔재 차단). `is_set()` Event proxy. cleanup-restart 패턴 (매 turn 새 인스턴스) | critical 모드 turn loop wrap | P-RAW 정합 + plan 009 hot-fix history |
...
본 모듈은 stdin/raw mode 통제라 P-RAW 결함 통로 6종 모두 발생 가능:
...
plan 008 hot-fix round + plan 009 hot-fix round 사례 누적 — `validation.md:342-360`.
...
- `docs/dev-docs/validation.md` P-RAW — raw mode 결함 통로 6종 + plan 008·009 사례 누적
- `docs/dev-docs/systems/orchestrator.md` — UI 호출자 wiring (run_turn / _run_session_*)
- `plan/completed/006-ui/01-plan.md`, `plan/completed/008-ui-polish/01-plan.md`, `plan/completed/009-user-synthesis-wiring/01-plan.md` — 누적 산출 history
