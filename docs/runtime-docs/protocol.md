...
```jsonc
{
  ...
  "meta": {
    ...
    "apply_status": "ok",                // kind=patch_applied일 때만: "ok" | "failed" | "no_fence" (all-or-nothing 채택, partial 폐기 — no_fence는 proposal에 search-replace 마커 0건 신호)
    "apply_error": null,                 // apply_status=failed 시 사유 (예: "search not found in wave_difficulty.py", "path outside workdir: ../etc/passwd"). no_fence 시 "no FILE: marker found in proposal"
    "files_changed": ["wave_difficulty.py"]  // apply_status=ok 시 적용된 파일 리스트, failed/no_fence 시 빈 리스트 [] (롤백되어 실 변경 0)
  }
}
```
...
| `patch_applied` | proposal 직후 항상 (apply 성공/실패/proposal에 fence 0건 모두) — `meta.apply_status`/`apply_error`/`files_changed` 동반 | `system` |
...
flowchart TD
    ...
    R2_6["**2.6.** apply_patches(workdir) — all-or-nothing<br/>각 patch FILE 경로를 workdir 내부로 강제<br/>(Path.resolve() + 외부 경로 차단)<br/>각 patch의 SEARCH를 파일에서<br/>정확 일치 검색 → REPLACE 치환<br/>SEARCH=&quot;&quot; + 파일 부재 → 신규 파일 생성<br/>(parent mkdir + write_text)<br/>1개라도 실패 시 전체 롤백<br/>(신규 파일은 unlink, 기존은 originals 복원)"]
    R2_7{"**2.7.** apply 성공?"}
...
| Patch SEARCH="" + 신규 파일 write IO 실패 (parent mkdir 실패 / write_text 실패 — 권한·디스크) | 동일 IO 실패 분기 진입 — best-effort 롤백 시 신규 파일은 `unlink`, 기존 파일은 originals 복원 (plan 014 Phase D) | `kind=patch_applied, apply_status=failed, apply_error="io error on <file>: <errno>", files_changed=[]` |
| 신규 파일 rollback `unlink` 실패 (권한·동시성) | rollback_failures 누적 + stderr 경고 + apply_error 메시지에 `(rollback also failed: ...)` 합성 (best-effort 신호 — 사용자가 수동 정리 필요할 수 있음) | `kind=patch_applied, apply_status=failed, apply_error="io error on <file>: <errno> (rollback also failed: <path>: <restore_errno>)", files_changed=[]` |
| Proposal에 search-replace 마커(`FILE:` + `<<<<<<< SEARCH` / `=======` / `>>>>>>> REPLACE`) 0건 (driver 형식 미준수 또는 의도적 코드 부재 응답) | apply 시도 0 + apply_status=no_fence 명시 발행 (silent skip 차단 — 다음 턴 driver R1 prompt에 `SYSTEM (patch_applied): apply_status=no_fence ...` 자가 교정 신호) | `kind=patch_applied, apply_status=no_fence, apply_error="no FILE: marker found in proposal", files_changed=[]` |
