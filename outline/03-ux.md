...
**`--interactive {full,critical,end-only}` 플래그** (default `critical`, Q18 ✅): 사용자 개입 강도 dial. 진입로 1·2 양쪽에서 동일하게 동작. default 동작이라 CLI 예시에는 보통 생략된다 (명시할 때만 표기).

```bash
$ dialectic run                              # 모드만 정함 → task부터 메뉴 (default --interactive critical)
$ dialectic run --task @tasks/wave/task.md   # task까지 정함 → 매핑·workdir만 default
$ dialectic run --task @tasks/wave/task.md --interactive critical  # 명시 (default와 동일)
```
...
**critical 모드 자동 진행 예시** (Turn 2 reviewer가 P0/P1=0인 경우 — Q18 ✅, Q6 = b):
